...
import { FiPlus, FiSend, FiX, FiCalendar, FiStar, FiClock, FiUsers, FiDollarSign, FiTrendingUp, FiAward, FiSearch, FiFilter } from 'react-icons/fi'
...
const SkillExchange = () => {
  ...
  const confirmBooking = async () => {
    try {
      const selectedSlot = new Date().toISOString() // In real app, user selects from available slots
      await firebaseRealtimeService.bookSession(selectedSession.id, user.id, selectedSlot)
      success(`Session booked! ${selectedSession.coinsCost} coins deducted`)
              ...
              {filteredSessions.map((session) => (
                ...
                >
                  <Card className="h-full hover:shadow-xl transition cursor-pointer">
                    ...
                    {/* Skill Info */}
                    <div className="mb-4">
                      <h4 className="text-lg font-bold text-gray-900 mb-2">{session.skillName}</h4>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                      ...
                      >
                        {session.teacherId === user?.id ? 'Your Session' : 'Book Now'}
                      </Button>
                    </div>
                  </Card>
                </motion.div>
                  ...
                  <div key={booking.id} className="p-4 border border-gray-200 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">Learner ID: {booking.learnerId}</p>
                ...
                {myBookings.asLearner.map((booking) => (
                  <div key={booking.id} className="p-4 border border-gray-200 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-semibold text-gray-900">{booking.skillName}</h3>
                        <p className="text-sm text-gray-600">Teacher ID: {booking.teacherId}</p>
                        <p className="text-sm text-gray-500">{booking.duration} minutes</p>
                      </div>
                      <div className="text-right">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input
              ...
              onChange={(e) => setNewSession({ ...newSession, coinsCost: parseInt(e.target.value) })}
            />
          </div>
          ...
          <Button variant="primary" className="w-full" onClick={handleCreateSession}>
            Create Session & Start Earning
          </Button>
        </div>
      </Modal>

      {/* Book Session Modal */}
      <Modal isOpen={showBookModal} onClose={() => setShowBookModal(false)} title="📅 Book Session">
        {selectedSession && (
          <div className="space-y-4">
            <div className="text-center">
              <h3 className="text-xl font-bold text-gray-900 mb-2">{selectedSession.skillName}</h3>
              <p className="text-gray-600">{selectedSession.description}</p>
              <div className="mt-4 p-4 bg-indigo-50 rounded-lg">
                <div className="text-3xl font-bold text-indigo-600 mb-2">{selectedSession.coinsCost} 💎</div>
            ...
            <div className="flex gap-3">
              ...
              <Button 
                ...
                disabled={userStats.coinsBalance < selectedSession.coinsCost}
              >
                {userStats.coinsBalance < selectedSession.coinsCost ? 'Insufficient Coins' : 'Confirm Booking'}
              </Button>
            </div>
          </div>
        )}
