...
import { FiEdit2, FiMessageSquare, FiUserPlus, FiAward, FiUsers, FiX, FiCheck, FiArrowLeft } from 'react-icons/fi'
...
const Profile = () => {
      ...
      try {
        ...
        if (paramUserId && paramUserId !== currentUserId) {
          const unsubscribe = firebaseRealtime.subscribeToUsers((users) => {
            const targetUser = users.find((u) => u.id === paramUserId)
            if (targetUser) {
              // Ensure user object has id field
              const userWithId = { ...targetUser, id: paramUserId }
              setUser(userWithId)
              setIsOwnProfile(false)
              setAvatarPreview(targetUser.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${paramUserId}`)
            }
            setIsLoading(false)
          })
          return () => unsubscribe?.()
          ...
          if (result.success && result.data) {
            ...
            setEditForm({
              ...
              bio: result.data.bio || '',
              avatar: result.data.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${currentUserId}`,
            })
            setAvatarPreview(result.data.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${currentUserId}`)
          } else {
  ...
  const loadFollowersList = async () => {
    ...
    try {
      ...
      const unsubscribe = firebaseRealtime.subscribeToFollowers(user.id, (followers) => {
        console.log('🔄 Real-time followers updated:', followers.length)
  ...
  const loadFollowingList = async () => {
    ...
    try {
      ...
      const unsubscribe = firebaseRealtime.subscribeToFollowing(user.id, (following) => {
        console.log('🔄 Real-time following updated:', following.length)
  ...
  const handleCancel = () => {
    ...
    setAvatarPreview(user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${authUser?.id}`)
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-indigo-50">
                ...
                followersList.map((follower) => (
                  <div key={follower.uid} className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1">
                      <img
                        src={follower.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${follower.uid}`}
                        alt={follower.name}
              ...
              {followingList.length === 0 ? (
                ...
                followingList.map((following) => (
                  <div key={following.uid} className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1">
                      <img
                        src={following.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${following.uid}`}
                        alt={following.name}
                        className="w-10 h-10 rounded-full object-cover"
                      />
                      <div className="flex-1 min-w-0">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="text-center pb-8 relative overflow-hidden">
          ...
          {/* Avatar */}
          <img
            src={user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${authUser?.id}`}
            alt={user?.name}
            className="w-32 h-32 rounded-full mx-auto border-4 border-white shadow-lg -mt-16 mb-4 object-cover"
          />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
              <p className="text-3xl font-bold text-green-600">{user?.certificates || 0}</p>
              <p className="text-xs font-medium text-green-700">Certificates</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white">
            <h3 className="text-lg font-bold mb-3 flex items-center gap-2">
              <FiAward size={20} /> Learning Streak
            </h3>
            <div className="flex items-end gap-2 mb-2">
              <span className="text-4xl font-bold">7</span>
              <span className="text-indigo-100 mb-1">days</span>
            </div>
            <p className="text-sm text-indigo-100">Keep it up! You're on fire 🔥</p>
          </Card>
