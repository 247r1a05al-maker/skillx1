...
const Explore = () => {
  ...
  const isPremiumUser = (u) => {
    ...
    const normalizedUsername = (u?.username || u?.name || '').toLowerCase()
    return premiumEmails.includes(normalizedEmail) || normalizedUsername === premiumUsername
  }
  ...
  useEffect(() => {
    ...
    const unsubscribe = firebaseRealtime.subscribeToUsers((firebaseUsers) => {
      ...
      let usersToDisplay = uniqueUsers && uniqueUsers.length > 0 ? uniqueUsers : demoUsers
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredUsers.map((user, index) => (
            ...
            >
              <Card className="relative hover:shadow-2xl transition-all duration-300 flex flex-col h-full group bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900">
                {/* Animated Border Effect */}
                <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 opacity-0 group-hover:opacity-20 blur transition-opacity duration-300 pointer-events-none" />
                ...
                {user.isOnline && (
                  <motion.div 
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="absolute top-4 right-4 flex items-center gap-1 bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-semibold z-10"
                  >
                ...
                {/* User Info */}
                <div className="flex flex-col items-center text-center mb-4 flex-1 relative z-10">
                  <motion.div
                  ...
                  >
                    <Avatar
                      ...
                      userId={user.id}
                      size="lg"
                      className="ring-4 ring-indigo-100 group-hover:ring-indigo-300 transition-all shadow-lg"
                    />
                  </motion.div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2 group-hover:text-indigo-600 transition-colors">
                    {user.name}
                  </h3>
                ...
                <div className="flex gap-2 relative z-20">
                  <Button
                    ...
                    variant="primary"
                    size="sm"
                    className="flex-1 flex items-center justify-center gap-1 pointer-events-auto"
                    onClick={() => {
                      console.log('View clicked for user:', user.id, user.name)
                  ...
                  <Button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 pointer-events-auto"
                    onClick={() => {
                      console.log('Chat clicked for user:', user.id, user.name)
                      navigate(`/inbox?user=${user.id}`)
                    }}
                  >
