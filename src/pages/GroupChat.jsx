...
import { FiArrowLeft, FiUsers, FiMessageSquare, FiX, FiSmile, FiSend, FiUserPlus, FiMenu, FiTrash2, FiShield } from 'react-icons/fi'
...
const GroupChat = () => {
  ...
  const isCurrentUserAdmin = members.find(
    (m) => m.userId === (authUser?.uid || authUser?.id)
  )?.role === 'admin'
  ...
  const handleRemoveMember = async (userId) => {
    ...
    if (window.confirm('Remove this member from the group?')) {
      try {
        const result = await firebaseRealtime.removeMemberFromGroup(
          groupId,
          userId,
          authUser.uid || authUser.id
        )
  ...
  const handlePromoteToAdmin = async (userId) => {
    ...
    if (window.confirm('Promote this member to admin?')) {
      try {
        const result = await firebaseRealtime.updateMemberRole(
          groupId,
          userId,
          'admin',
          authUser.uid || authUser.id
          ...
          >
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {members.map((member) => {
                ...
                const isCurrentUser = member.userId === (authUser?.uid || authUser?.id)
                return (
                  <div key={member.userId} className="p-3 hover:bg-gray-50 rounded-lg transition group">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {/* Admin controls - only show to admins, not for themselves */}
