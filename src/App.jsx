...
// Streak Updater - Update daily streak on app initialization/login
const StreakUpdater = () => {
  const { user, isAuthenticated } = useAuthStore()
  ...
  useEffect(() => {
    ...
    const updateStreak = async () => {
      try {
        const result = await firebaseRealtime.updateDayStreak(userId)
        if (result.success && result.isNewDay) {
          console.log('🔥 Streak updated! Current streak:', result.data.currentStreak)
        }
      } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600">
        <div className="text-center">
          ...
          <p className="text-white text-lg font-semibold">Loading SkillEx...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <ThemeProvider>
      ...
      <Router basename={import.meta.env.BASE_URL}>
        <Routes>
          {/* Merge accounts - public route, no layout */}
